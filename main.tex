...
\begin{frame}{Classical Ion Trapping}
	FILL IN WITH EXAMPLE TRAP WITH PARAMS AND IMAGE
	This is a Linear Paul Trap, with
	\begin{center}
		$-(\alpha + \beta) = \gamma$\\
		$\alpha'=-\beta'$
	\end{center}
...
\begin{frame}{Quantum Ion Trapping}
	...
	\begin{center}
		$\dot{\hat{x}}_H = \bigfrac{1}{i\hbar}\lbrack \hat{x},\hat{H}^{(m)}\rbrack = \bigfrac{\hat{p}}{m}$ \\[4pt]
		$\dot{\hat{p}}_H = \bigfrac{1}{i\hbar}\lbrack \hat{p},\hat{H}^{(m)}\rbrack = -mW(t)\hat{x}$
	\end{center}
	which combine into a Mattheiu equation!
	\[\ddot{\hat{x}}_H+W(t)\hat{x}_H = 0\]
	\[\ddot{\hat{x}}_H+ \bigfrac{\omega_{rf}^2}{4}\lbrack a_x-2q_x\cos(\omega_{rf}t) \rbrack \hat{x}_H = 0\]
...
\begin{frame}{Quantum Ion Trapping}
	...
	\[\hat{C}_S(t)\hat{U}(t)\ket{n=0} = \hat{C}_S(t)\ket{n=0,t}=0\]
\end{frame}
\begin{frame}{Quantum Ion Trapping}
	...
	write ground state lowest order approx
\end{frame}
